...
**Tech Stack:** React 18, Vite 5, TypeScript 5, `@xyflow/react` (React Flow 12), zustand, Vitest + happy-dom for the unit tests the spec mandates. Deployed on Vercel.
...
- **Anthropic-only for v1.** "A provider abstraction is exactly the speculative plumbing the scope contract exists to kill." No provider abstraction layer.
...
└── src/
    ...
    ├── design/
    │   └── tokens.css           # design tokens (theme 1)
    ├── canvas/
...
**Files:**
- Create: `package.json`, `tsconfig.json`, `vite.config.ts`, `vitest.config.ts`, `index.html`, `src/main.tsx`, `src/App.tsx`, `.gitignore`, `LICENSE`
...
- Produces:
  ...
  function streamMessage(
    opts: { apiKey: string; messages: Message[]; signal?: AbortSignal },
...
**Interfaces:**
- Produces: CSS custom properties for color, a modular type scale, and a spacing scale, on `:root`. All later components consume these variables — no hardcoded colors/sizes after this task.

- [ ] **Step 1: Author `src/design/tokens.css`**

```css
:root {
  /* color */
  --color-bg: #0f1115;
  --color-surface: #191d24;
  --color-surface-2: #21262f;
  --color-border: #2c333d;
  --color-text: #e6e9ee;
  --color-text-dim: #9aa4b2;
  --color-accent: #7c9cff;
  --color-user: #8fb8ff;
  --color-assistant: #c7d0dd;
  --color-danger: #ff8f8f;
...
Add near the top: `import './design/tokens.css'`

- [ ] **Step 3: Manual verification**

Run `npm run dev`: the app background/text now use the dark token palette. (Components still use inline styles — rebuilt next task.)

- [ ] **Step 4: Commit**

```bash
git add src/design/tokens.css src/main.tsx
git commit -m "feat(design): design tokens — color, type scale, spacing"
```
...
- Create: `src/canvas/ThreadNode.css` (co-located component styles using tokens)

**Interfaces:** no signature changes — visual rebuild only. Replace every inline hardcoded color/size with token variables; move styles into a co-located CSS file with class names.

- [ ] **Step 1: Create `src/canvas/ThreadNode.css`**
...
.thread__scroll { max-height: 320px; overflow-y: auto; padding: var(--space-2); }
.thread__quote {
  margin: var(--space-2);
...
.message { padding: var(--space-1) var(--space-2); white-space: pre-wrap; font-size: var(--text-sm); line-height: 1.5; }
.message[data-role="user"] { color: var(--color-user); }
.message[data-role="assistant"] { color: var(--color-assistant); }
.composer { display: flex; gap: var(--space-1); padding: var(--space-2); border-top: 1px solid var(--color-border); }
.composer textarea {
  flex: 1; resize: none; background: var(--color-surface-2); color: var(--color-text);
  border: 1px solid var(--color-border); border-radius: var(--radius-sm); padding: var(--space-2);
  font-family: var(--font-sans); font-size: var(--text-sm);
}
.btn { background: var(--color-surface-2); color: var(--color-text); border: 1px solid var(--color-border);
  border-radius: var(--radius-sm); padding: var(--space-1) var(--space-2); cursor: pointer; font-size: var(--text-xs); }
.btn:hover { border-color: var(--color-accent); }
```

- [ ] **Step 2: Replace inline styles with classes** in `ThreadNode.tsx` (import `./ThreadNode.css`; `className="thread"`, `thread__scroll`, `thread__quote`, branch button `btn`), `MessageView.tsx` (`className="message"`), and `Composer.tsx` (`className="composer"`, button `btn`). Remove the corresponding inline `style` objects.

- [ ] **Step 3: Style edges** in `CanvasView.tsx`: give each derived edge `type: 'smoothstep'`, `animated: false`, and `style: { stroke: 'var(--color-border)', strokeWidth: 2 }`; set `<ReactFlow ... defaultEdgeOptions>` accordingly. Set the `<Background>` color to `var(--color-border)`.

- [ ] **Step 4: Manual verification (before/after shot)**

Run `npm run dev`. The node, composer, messages, and edges now read as one system. Capture the before/after (skeleton vs. systematized) — this is the theme's post.

- [ ] **Step 5: Commit**

```bash
git add src/canvas src/components/MessageView.tsx src/components/Composer.tsx
git commit -m "feat(design): rebuild node, edge, composer on design tokens"
```
...
**Files:**
- Modify: `src/canvas/CanvasView.tsx` (use `useReactFlow` for animated camera; animate edge draw), `src/canvas/ThreadNode.css` (node sprout keyframes), `src/state/store.ts` (expose `lastBranchedId` so the view can animate the newcomer)
...
- [ ] **Step 3: Sprout keyframes in `ThreadNode.css`**
...
Restyle `KeyEntry.tsx` using tokens: centered card on `--color-surface`, product one-liner ("An AI conversation on an infinite canvas"), the key field, the privacy line (already present, keep verbatim), and a "Try the sample instead" affordance stub (wired in Task 24). No new logic beyond the sample stub calling a passed `onTrySample` (optional prop, unused until Task 24).
...
Render `errors[thread.id]` as a `--color-danger` banner with a Retry button (`onClick={() => useStore.getState().retry(thread.id)}`). Add a `ref` to the scroll container and an effect that scrolls to bottom when `thread.messages` changes; when the user has scrolled up, show a "↓ latest" button that jumps to bottom.
...
- [ ] **Step 2: Mount the toolbar** as a React Flow `<Panel position="top-right">` in `CanvasView.tsx`. Add `.toolbar` styles (tokens) to a stylesheet.
...
| Anthropic-only, no provider abstraction | 7 (single `DEFAULT_MODEL` constant) |
